...
| 35 | a uniqueness assertion protects correct data | Phase 9B's first version compared bare bundle **names** and fired on two legitimate bundles at different origins. Corrected to compare full bundle **paths** before being relied on; recorded because a check that fires on correct data gets disabled | [8.2](#step-82--rolling-origin-backtest) |
| 36 | 10.1's inputs include a holding cost, an ordering cost and a truck capacity | **none of the three exists** in the 49-table schema. The objective carries only per-unit freight; holding and ordering are injected parameters defaulting to 0.0, leaving the schedule's timing unidentified | [10.1](#step-101--delivery-schedule-lp) |
| 37 | 10.1's verify gate discriminates | **it cannot fail**: "zero holding cost" and "no capacity constraint" are both true of every part (`max_volume_cap` constant 5,000 vs largest requirement 2,102), and with c_hold = 0 ordering first costs what ordering last costs. **8th gate that cannot fail**; replaced by five gates each demonstrated failing | [10.1](#step-101--delivery-schedule-lp) |
| 38 | a rolling plan gives a forward requirement horizon | `part_demand_weekly` carries **one** week per part-plant 30 days out, at 12 as-of dates ~9.5 months apart; `production_plan` is **entirely retrospective** (`target_period` 1-8 days before `recorded_ts`, 0.00% forward in both worlds). The deliverable schedule is single-period | [10.1](#step-101--delivery-schedule-lp) |
| 39 | the stock balance is a 10.1 constraint | **dropped**, with the safety-stock floor: no opening level exists in any usable form (all-zero table; 97.5% negative snapshots; 0.33% cumsum agreement). A balance mode behind an injected `i0` is a labelled sensitivity, never a deliverable | [10.1](#step-101--delivery-schedule-lp) |
| 40 | 10.2's qualification lead time is a binding constraint | **INERT**: `qualification_status` is constant "qualified" and `is_approved` constant 1, so nothing is ever unqualified. Implemented against an injectable cutoff and reported as unenforceable on this data | [10.2](#step-102--allocation-enumeration-then-milp) |
