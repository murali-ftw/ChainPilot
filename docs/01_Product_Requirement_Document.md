...
| NFR-03 | **Compute stays modest.** The full architecture at `d=64`, `L=4` is ~773K parameters / ~1.4 GFLOP full-graph forward pass (`project_HADES.md` Part 7) — trainable on a single consumer GPU or CPU in reasonable time; no distributed training infrastructure is required or planned |
