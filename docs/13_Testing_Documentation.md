...
| `SPOFService.traverse` (`analytics/spof_service.py`) | Given a known graph fixture, reachable product/order counts and order-value share match hand-computed expected values | FR-SPOF-01 |
| `SegmentationService.cluster` (`analytics/segmentation_service.py`) | Given the same embedding set and `k`, two consecutive runs produce cluster assignments with Adjusted Rand Index above a documented stability threshold | FR-SEG-01 |
| `EvaluationService.persist_metrics` — `task` column | Given a metrics payload for a supplementary task (e.g., `lead_time_regression`), the persisted row's `task` matches the submitted value; a payload omitting `task` defaults to `risk_prediction` | Document 5 §6.23 |
| `Transformer2.attend` — same-type constraint (Fix A) | Given a mixed-node-type batch, attention weights between differently-typed nodes are exactly zero; nonzero only within same-type pairs | `updates/Supplier_Risk_Prediction.md` §6.3 — **Not committed — Phase 2 (early April 2027) or later; stretch-only before then, and only after RG-01 is solid** |
| `Transformer1.train` — dual-run methodology (Fix B) | Given the shallow-regularized run's output, asserts it is never written to the path that produces the Markov Claim A report; only the unbiased run's weights reach that path | `updates/Supplier_Risk_Prediction.md` §6.2 — **Not committed — Phase 2 (early April 2027) or later; stretch-only before then, and only after RG-01 is solid** |
| `LinkPredictionService.evaluate` | Given a held-out edge split (80/10/10), computes AUC-ROC and Average Precision; comparable to the Kosasih & Brintrup (2021) / Aziz et al. (2021) 0.877 AUC benchmark | `updates/New_Features.md` F-09 |

**Delivery Phase:** Phase 1 (auth, entity, prediction, explainability, risk formula, confidence, categorization, evaluation, governance, SPOF, segmentation, `task` column units); Phase 2 (approval, chatbot intent classification, recommendation similarity, decision routing/validation/trace, optimizer, allocation, link-prediction units); Not committed — Phase 2 (early April 2027) or later; stretch-only before then, and only after RG-01 is solid (same-type attention constraint, dual-run methodology units)
