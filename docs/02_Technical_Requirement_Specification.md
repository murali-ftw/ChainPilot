...
- **Phase 1:** Auth service, PostgreSQL, graph construction pipeline, GNN-Transformer inference service, REST API layer, React dashboard, D3 graph visualization.
- **Phase 2:** Adds RAG service (vector database + retriever), LLM orchestration service, chatbot service, MCP execution layer, ERP/procurement integration adapters, notification service.
...
A layered, service-oriented architecture matching the problem statement's six logical layers, implemented as a set of backend services behind a single API gateway, a Python ML/AI subsystem, and a React single-page frontend. Phase 1 ships four backend services; Phase 2 adds three more without altering Phase 1 service boundaries — services are additive, not replaced.
...
flowchart TB
    ...
    class RAG,LLMS,CHAT,MCP,NOTIFY,ERP,LLMAPI,SLACK phase2;
```

*Nodes and edges marked with `*` or shaded purple are Phase 2. Unshaded nodes are Phase 1.*
...
| Layer 2 — GNN × Transformer | GNN-Transformer Inference Service | Phase 1 |
| Layer 3 — RAG | RAG Retrieval Service, Vector DB | Phase 2 |
| Layer 4 — LLM | LLM Orchestration Service | Phase 2 |
| Layer 5 — MCP Servers | MCP Execution Service, Notification Service | Phase 2 |
...
| GNN-Transformer Inference Service | Loads trained model artifact, serves scoring requests, produces explanation subgraphs and embeddings | Phase 1 |
| RAG Retrieval Service | Embeds and indexes evidence documents, serves hybrid search queries | Phase 2 |
| LLM Orchestration Service | Coordinates prompt assembly, RAG retrieval calls, LLM API calls, business-rule validation of proposed actions | Phase 2 |
...
| GNN architectures | GraphSAGE / GAT baseline, Heterogeneous Graph Transformer | Phase 1 |
...
flowchart TB
    ...
    classDef phase2 fill:#f5e6ff,stroke:#8855cc;
    class RAGC,LLMC,MCPC,VECC phase2;
```
...
| Action authorization | Mandatory human-approval record before any MCP execution | Phase 2 |
...
Same as Document 1, Section 13, at the infrastructure level: PostgreSQL, PyTorch/PyTorch Geometric, GNNExplainer, React, D3.js, FastAPI, pgvector/Weaviate, LLM API, LangGraph, MCP servers, ERP sandbox/mock, Slack/email API.

## 16. Risks

| ID | Risk | Mitigation | Delivery Phase |
|---|---|---|---|
| TR-01 | Service sprawl (7+ containers by Phase 2) increases operational complexity for a solo developer | Docker Compose with clear per-service README; health checks surface failures early | Phase 2 |
