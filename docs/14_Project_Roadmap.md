...
This document defines the delivery roadmap: milestones, sprint plan, feature dependencies, implementation order, critical path, timeline, and risk register, sequencing every requirement from Document 1 across Phase 1 (15 weeks) and Phase 2 (target: early April 2027).

## 2. Scope

**Canonical date reconciliation:** this document's Phase 1 completion date is **2026-11-15** — the same date used consistently across `updates/Supplier_Risk_Prediction.md`, `updates/Other_Tools.md`, and `updates/New_Features.md`. This supersedes an earlier 2026-11-01 date that appeared only in this document; every milestone, sprint date, and Gantt reference below has been updated to match.

Phase 1: starting 2026-07-20, targeting completion **2026-11-15** (extended from the original 15-week/2026-11-01 window by two weeks, absorbing the rebalanced scope below — `updates/Other_Tools.md` Section 10.5). Phase 2: extends Phase 1 from 2026-11-16 through Phase 2 sign-off (target: early April 2027, per the same two-week shift), per Document 1's delivery strategy. Dates are planning estimates from the current date (2026-08-02), not contractual deadlines beyond the Phase 2 early April 2027 target stated in the problem statement.

**Rebalanced Phase 1 scope (per `updates/Other_Tools.md` Section 10.5):** adds single-point-of-failure analysis, supplier segmentation, geographic/spend concentration, and new-supplier onboarding demo (Document 1, Section 2.1, items 15–19), plus a thin single-supplier RAG+LLM+Chatbot vertical slice (item 20) — see Section 6, Sprint S8 below. **Explicitly excluded from the Nov-15 window:** the Layer 2 architecture upgrade (**Not committed — Phase 2 (early April 2027) or later; stretch-only before then, and only after RG-01 is solid**) and demand forecasting (**Not in scope — documented as a future extension idea only, no committed delivery phase**) — neither was ever part of this roadmap's sprint plan, and neither is added by this reconciliation.
...
| M5a — Rebalanced Scope Complete                                                    | 2026-11-08           | Analytics screen (SPOF, segmentation, geographic/spend concentration), new-supplier onboarding demo, and the thin single-supplier RAG+LLM+Chatbot slice (Document 1 §2.1 items 15–20) functional against live APIs |
| **M6 — Phase 1 MVP Sign-off**                                                | **2026-11-15** | Document 13 Phase 1 test suite green (incl. new SPOF/segmentation/`task`-column test cases, Document 13 §6); UAT scenarios (Priya, Meera, Arjun, Devika/Arjun Model Comparison) pass |
| M7 — RAG + LLM Live (full production scope)                                       | 2026-12-27           | Vector DB populated, RAG retrieval + LLM explanation generation working at full production scope (the thin Phase 1 slice, M5a, already proved the end-to-end path) |
| M8 — Chatbot Live (full production scope)                                          | 2027-01-24           | Chatbot end-to-end per Document 4 §9 (multi-turn history, in-chat approval), integrated into dashboard |
| M9 — Simulator + Recommender + Decision Intelligence + Optimizer + Allocation Live | 2027-02-21           | What-if simulation, alternative-supplier recommendation, Decision Intelligence routing (FR-DEC-01), OR-Tools CP-SAT safety-stock/PO-split decisions (FR-OPT-01), and customer allocation as a constrained optimization problem (FR-CUST-02) functional                  |
| M10 — Agentic Layer Live                                                           | 2027-03-21           | Approval workflow (incl. optimizer/allocation sources, decision-trace population FR-DEC-03), MCP execution against sandbox ERP (never live production ERP), alerts, and notifications functional                                                                                             |
| **M11 — Phase 2 Sign-off**                                                   | **2027-04-04** | Full Document 13 test suite (Phase 1 + Phase 2) green; all UAT personas (incl. Neha) pass. Does **not** include the Layer 2 upgrade or demand forecasting — neither is scheduled to this or any milestone. |

## 6. Sprint Plan

### Phase 1 (9 sprints, 2026-07-20 → 2026-11-15)
...
| S8     | 2026-10-26 – 2026-11-08 | Rebalanced scope (M5a)                                      | Analytics screen — SPOF, segmentation, geographic/spend concentration (FR-SPOF/SEG/GEO/SPEND, Document 8 `analytics/` module); new-supplier onboarding demo (FR-ONBOARD-01); thin single-supplier RAG+LLM+Chatbot vertical slice (Document 1 §2.1 item 20) |
| S9     | 2026-11-09 – 2026-11-15 | Sign-off                                                    | UAT (Document 13 §13, Priya/Meera/Arjun scenarios plus rebalanced-scope items); bug-fix buffer; Phase 1 demo prep                                                                                                                                                                                                                                                                                               |

### Phase 2 (10 sprints, 2026-11-16 → 2027-04-04)

| Sprint | Dates                    | Focus                                                                         | Key Deliverables (Requirement IDs)                                                                                                                                                                                                                                                                                  |
| ------ | ------------------------ | ----------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| S10    | 2026-11-16 – 2026-11-29 | Vector DB                                                                     | Evidence schema, chunking, embedding pipeline (FR-RAG-01, Document 7 §5–7) — full production scope, extending the thin single-supplier slice already shipped in S8/M5a                                                                                                                                                       |
| S11    | 2026-11-30 – 2026-12-13 | RAG retrieval                                                                 | Hybrid search, ranking, filtering (FR-RAG-02–04)                                                                                                                                                                                                                                                                   |
| S12    | 2026-12-14 – 2026-12-27 | LLM orchestration                                                             | Explanation generation, business-rule validation (FR-LLM-01–04)                                                                                                                                                                                                                                                    |
| S13    | 2026-12-28 – 2027-01-10 | Buffer / low-velocity sprint (holiday period)                                 | Documentation catch-up, technical debt, exploratory chatbot prototyping                                                                                                                                                                                                                                             |
| S14    | 2027-01-11 – 2027-01-24 | Chatbot                                                                       | Full production scope — multi-turn history, in-chat approval, intent classification, retrieve-then-generate pipeline, chat UI (FR-CHAT-01–05); the thin one-turn Q&A slice already shipped in S8/M5a                                                                                                                                                    |
| S15    | 2027-01-25 – 2027-02-07 | Explainability overlay + Simulator                                            | Full overlay (FR-EXP-01–03); what-if re-inference (FR-SIM-01–04)                                                                                                                                                                                                                                                  |
| S16    | 2027-02-08 – 2027-02-21 | Decision Intelligence + Recommender + Optimizer + Trend Timeline + Allocation | Decision Intelligence Service — routing, policy validation (FR-DEC-01–02); alternative-supplier recommendation (FR-REC-01–04); trend timeline (FR-TREND-01–03); OR-Tools CP-SAT optimization engine incl. customer-allocation solver (FR-OPT-01–03); customer allocation as constrained optimization (FR-CUST-02–03). Does not include the Layer 2 upgrade or demand forecasting — neither is scheduled here. |
| S17    | 2027-02-22 – 2027-03-07 | Approval + MCP execution + Decision Trace                                     | `action_requests`/`action_log` incl. `decision_trace` (Document 5 §6.17–18); approval workflow (FR-MCP-03–04); decision-trace population (FR-DEC-03); MCP/ERP sandbox adapter only, never live production ERP (Document 4 §12–13)                                                                                                        |
| S18    | 2027-03-08 – 2027-03-21 | Alerts + Notifications                                                        | Threshold config, alert evaluation (FR-MCP-05–06); Slack/email delivery (Document 4 §14)                                                                                                                                                                                                                          |
| S19    | 2027-03-22 – 2027-04-04 | Sign-off                                                                      | Full Document 13 test suite (Phase 1 + 2); all UAT personas; security review (Document 12 Phase 2 controls); final demo prep                                                                                                                                                                                        |
...
gantt
    ...
    Dashboard + Rebalanced Scope + UAT (M6) :milestone, cp5, after cp4, 35d
    section Phase 2
...
Chatbot UI, Explainability Overlay, Simulator, Recommender, Decision Intelligence, Optimizer, Customer Allocation, and Trend Timeline (Sprints 14–16) run **off** the critical path in parallel with/around it, since none of them blocks the Approval → MCP → ERP chain that gates M11 — but all must still complete before M11 sign-off per Document 1's Phase 2 scope. The architecture ablation, Risk Intelligence (confidence/weighted formula/categorization), and evaluation/governance persistence (Sprints S4–S5) sit **on** the Phase 1 critical path since Dashboard (S6) and M6 sign-off depend on the trained model being available, but add no new critical-path stage of their own — they extend the existing "GNN Training" / "Inference API" stages rather than following them.
...
gantt
    ...
    M5a Rebalanced Scope Complete                :milestone, 2026-11-08, 0d
    M6 Phase 1 MVP Sign-off                      :milestone, 2026-11-15, 0d
    section   
    M7 RAG + LLM Live (full scope)                :milestone, 2026-12-27, 0d
    M8 Chatbot Live (full scope)                  :milestone, 2027-01-24, 0d
    M9 Simulator/Recommender/DI/Opt/Alloc Live   :milestone, 2027-02-21, 0d
    M10 Agentic Layer Live                       :milestone, 2027-03-21, 0d
    M11 Phase 2 Sign-off                         :milestone, 2027-04-04, 0d
```
...
gantt
    ...
    Rebalanced Scope (S8)                        :2026-10-26, 14d
    Phase 1 UAT/Sign-off (S9)                    :2026-11-09, 7d
    Phase 1 Sign-off (M6)                        :milestone, 2026-11-15, 0d
    section   
    Vector DB (S10)                              :2026-11-16, 14d
    RAG Retrieval (S11)                          :2026-11-30, 14d
    LLM Orchestration (S12)                      :2026-12-14, 14d
    Buffer (S13)                                 :2026-12-28, 14d
    Chatbot (S14)                                :2027-01-11, 14d
    Overlay + Simulator (S15)                    :2027-01-25, 14d
    Recommender + Optimizer + Allocation (S16)   :2027-02-08, 14d
    Approval + MCP (S17)                         :2027-02-22, 14d
    Alerts + Notifications (S18)                 :2027-03-08, 14d
    UAT (S19)                                    :2027-03-22, 14d
    Phase 2 Sign-off (M11)                       :milestone, 2027-04-04, 0d
```
...
| RM-02 | Per-person velocity assumption (Document 11 §3) proves optimistic, or a cross-team handoff slips                                                                              | High                                                                                                                                                                 | Medium                    | 2-week sprint granularity makes slippage visible early; S13 buffer sprint in Phase 2 absorbs schedule drift from Phase 1 spillover                                        | S13 (Phase 2)          | Both           |
| RM-03 | Holiday period (S13, Dec 2026–Jan 2027) reduces effective velocity                                                                                                                      | High                                                                                                                                                                 | Low                       | Sprint explicitly scoped as low-velocity/buffer, not feature-critical                                                                                                     | Built into S13         | Phase 2        |
| RM-04 | ERP sandbox unavailable or harder to integrate than expected (Document 1, R-05)                                                                                                | Medium                                                                                                                                                               | Medium                    | S17 scoped with adapter abstraction (Document 8 §5,`mcp_execution/adapters/`) allowing a mock fallback without redesign                                                | Absorbed in S17–S18   | Phase 2        |
| RM-05 | Phase 2 feature parallelism (Section 9) creates integration surprises at S19                                                                                                   | Medium                                                                                                                                                               | Medium                    | Each feature (Overlay, Simulator, Recommender, Trend) integrated incrementally against the live API (Document 9) as built, not batch-integrated at the end                | Absorbed in S19        | Phase 2        |
...
| RM-08 | Optimizer + Allocation added to S16 alongside Recommender/Trend Timeline increases that sprint's scope                                                                         | Medium                                                                                                                                                               | Medium                    | All four are independent, parallelizable, off-critical-path features (Section 9); S19 sign-off buffer absorbs any S16 spillover, consistent with RM-05                    | Absorbed in S19        | Phase 2        |
| RM-09 | Risk Intelligence Service (S5) and Decision Intelligence Service (S16–S17) are new services that could be under-scoped as "just formatting" rather than real integration work | Both appear as explicit sprint deliverables with their own requirement IDs (FR-RISKINT-*, FR-DEC-*) below, not folded silently into Prediction/Approval line items | Absorbed in S5 / S16–S17 | Phase 1 / Phase 2                                                                                                                                                         |                        |                |
| RM-10 | Customer allocation's reframe from ranking to OR-Tools optimization (S16) requires supplier/warehouse/factory capacity data to already be populated and correct                | Capacity fields (`capacity_score`, `capacity_units`, `capacity_units_per_day`) are Phase 1 schema (Document 5), seeded and validated well before S16           | Absorbed in S16           | Phase 2                                                                                                                                                                   |                        |                |
| RM-11 | Rebalanced-scope sprint (S8, extended from 7 to 14 days) absorbs new Tier 1 features (SPOF/segmentation/concentration/onboarding demo) plus a thin RAG+LLM+Chatbot slice — genuinely new scope, not a renamed existing sprint | Medium | Medium | Tier 1 total effort is under a week combined (`updates/New_Features.md` §9); the RAG/LLM/Chat slice is deliberately thin (one supplier, one explanation, one-turn Q&A per `updates/Other_Tools.md` §10.5), leaving margin inside the 14-day window | Absorbed in S8 | Phase 1 |
