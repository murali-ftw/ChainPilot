...
-- Mechanism J (chain-length heterogeneity): the upstream supplier chain.
-- One row per hop, directed downstream -> upstream, so a tier-1 head reaches its
-- tier-2 source, that source reaches tier-3, and so on. Mechanism A truncates this
-- at max_visible_tier: a hop is emitted only when BOTH endpoints survive truncation,
-- so the observable graph terminates cleanly instead of dangling into the hidden
-- network. Absent entirely in variants without Mechanism J.
CREATE TABLE supplier_upstream (
    ...
    upstream_tier         SMALLINT NOT NULL,                       -- tier of the upstream node; 1 = product-facing
    created_at            TIMESTAMPTZ NOT NULL DEFAULT now(),      -- validity window start
...
CREATE INDEX idx_supplier_upstream_supplier  ON supplier_upstream (supplier_id);
CREATE INDEX idx_supplier_upstream_upstream  ON supplier_upstream (upstream_supplier_id);
CREATE INDEX idx_supplier_upstream_tier      ON supplier_upstream (upstream_tier);
