...
rate = np.exp(r.normal(P['demand_rate_mu_log'], P['demand_rate_sigma_log'], NCH))   # units/week baseline
...
safety = np.maximum(1, (rate * ss_w)).astype(np.int64)
rop = np.maximum(safety + 1, (rate * lead_wk + safety
                 + P['service_z'] * rate * np.sqrt(np.maximum(lead_wk, 1)) * P['demand_cv'])).astype(np.int64)
upto = np.maximum(rop + 1, (rop + rate * cover)).astype(np.int64)
lot = np.maximum(1, (rate * 0.5)).astype(np.int64)

PP = CP * NPL + CL; NPP = NP * NPL
PP_SAFETY = np.bincount(PP, weights=np.where(cold, 0.0, np.maximum(1, rate * ss_w)), minlength=NPP)
...
for t in range(T):
    ...
    dem = rate * SEAS[t] * TREND[t] * plant_f * (1 + dem_state[t][CL]) * r.lognormal(0, P['demand_cv'], NCH)
