# HADES Model-Development Prototype — RGCN+Attn vs RGCN+RelEmb, Matched-Parameter Pilot

**Scope:** train RGCN+attn ("Option 1", `reports/rgcn_attn_pilot.md`'s architecture) at a
matched parameter budget for the first time, and build + train a new sixth architecture —
RGCN+relation-embedding-attn ("Option 3") — also at a matched budget, both compared against
...
confirmed at a parameter count no longer confoundable with HGT's own. **RGCN+relemb ("Option
3"), tested here for the first time, does not clearly improve on RGCN+attn** — the paired
...
extra relation-identity signal Option 3 adds costs more parameters (388 more at matched hidden,
...
constraint — unsurprising, since Option 3's extra relation-embedding parameters
(`relation_embed_dim=16`, unchanged/not searched) are a small, roughly hidden-independent
addition on top of Option 1's own parameter curve.
...
## Before/after: RGCN+attn, fixed-d=64 pilot vs this round's matched-d
...
## Sign-consistency — RGCN+attn-matched vs RGCN+relemb-matched (PAIRED bootstrap)
...
**Option 1 (RGCN+attn): held, essentially unchanged.** Its fixed-d pilot win over HGT on delay
...
**Option 3 (RGCN+relemb): a real but smaller win than Option 1, not an improvement over it.**
...
its keep on this dataset. The simpler, cheaper Option 1 remains the better default.
...
third and most expensive of the RGCN-attention hybrid designs) not implemented.
