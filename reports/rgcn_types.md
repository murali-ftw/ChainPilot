...
**Scope:** for each of plain RGCN and its three attention hybrids (RGCN+Attention "Option 1",
RGCN+BasisAttn "Option 2", RGCN+RelEmbedding "Option 3") — what it is, its matched-parameter
...
## RGCN+Attention — "Option 1" (`ml/models/rgcn_attn_encoder.py`)
...
## RGCN+BasisAttn — "Option 2" (`ml/models/rgcn_battn_encoder.py`)
...
softmax as Option 1 runs over the concatenated pool per destination node.
...
**Impact's std (0.0559) is an order of magnitude wider than either other hybrid's** (Option
1: 0.0058, Option 3: 0.0071) — driven by a single outlier seed (seed1: impact=0.8016, vs.
0.9433/0.9468/0.9464/0.9189 for the other four). This pattern repeats at L1 (impact std 0.0329,
again driven by one low seed). **RGCN+BasisAttn is measurably less seed-stable on impact than
either of the other two hybrids** — the most expressive, most expensive design (two full basis
pools) also shows the most fragile training dynamics on this task, at this parameter budget.
...
Same qualitative shape as Option 1 — impact rises toward L4 — but every impact cell's std is
markedly wider than the corresponding Option 1/3 cell, the seed-instability pattern above
...
| **Shipment** | 0.932 | 0.936 | 0.916 | **0.889** | **Falling** (anti-smoothing, milder than Option 1) |

---

## RGCN+RelEmbedding — "Option 3" (`ml/models/rgcn_relemb_encoder.py`)

**What it is.** Extends Option 1's shared-scorer attention with one extra additive term: a
...
Option 2's second full basis pool.
...
depth — 0.935→0.788 for Option 1, 0.932→0.889 for Option 2, 0.935→0.767 for Option 3) — the
...
| RGCN+Attn / Option 1 (matched-d=128, num_bases=10) | 752,211 | 0.8105–0.8118 | 0.7971–0.7982 | 0.9365–0.9387 | `rgcn_matched_pilot.md`; reproduced this round |
| RGCN+BasisAttn / Option 2 (hidden=110, num_bases=9, num_bases_attn=16) | 738,273 | 0.8113 | 0.7956 | 0.9114 (unstable, std 0.056) | This round (new) |
| RGCN+RelEmbedding / Option 3 (matched-d=128, num_bases=10, relemb=16) | 752,599 | 0.8100–0.8105 | **0.7988–0.7991** | 0.9366–0.9369 | `rgcn_matched_pilot.md`; reproduced this round |

**The overall ranking established across every round holds, and this round doesn't overturn
it.** RGCN+Attn (Option 1) remains the strongest single architecture found in this project —
...
count before matching and near-parity after. Option 3 (RelEmbedding) is a close second,
essentially tied with Option 1 on every task (`rgcn_matched_pilot.md`'s paired comparison:
all three tasks flip sign across seeds) at a marginally higher parameter cost for the added
relation-identity signal — not a clear win over the simpler Option 1. **Option 2 (BasisAttn),
new this round, does not improve on either simpler hybrid** — its delay/shortage numbers are
comparable to Options 1/3, but its impact result is both no better on average (0.9114 vs.
...
shortage most cleanly** (0.7976, matching Option 3's 0.7988–0.7991 within noise) but loses
...
**Recommendation, unchanged from `rgcn_matched_pilot.md`, now reinforced:** RGCN+Attn (Option
1) remains the best-supported candidate for any further work in this line — cheapest of the
three hybrids to build and train, most stable across seeds, and no other variant tried across
four rounds of experimentation (plain RGCN, and Options 1–3) beats it on more than one task.
